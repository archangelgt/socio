...
5. **Live mode + Advanced Access** for Instagram `comments` is required for Meta to push comments from the public. In Development, Meta only notifies for people with an app role. Socio also polls Graph every ~30s and auto-syncs from Moderation so comments still land without webhooks.
6. For Facebook Login for Business, create a Configuration in the App Dashboard and set `META_LOGIN_CONFIG_ID` (OAuth uses `config_id` instead of raw `scope`). Fallback scopes when unset: `pages_show_list`, `pages_read_engagement`, `pages_manage_engagement`, `pages_manage_metadata`, `pages_messaging`, `instagram_basic`, `instagram_manage_comments`, `instagram_manage_messages`, `business_management`.
7. Use a Facebook Page with a linked Instagram professional account. App Review is required before production hide works.
